...
%
%   6. HANDLE MISSING DATA PROACTIVELY - CRITICAL REQUIREMENT:
%      ⚠️ THE GENERATED REPORT MUST BE FINAL AND COMPLETE WITH NO PLACEHOLDERS
%
%      Before generating the report, you MUST verify ALL required information
%      is provided. If ANY data is missing, STOP and REQUEST it explicitly.
%
%      MANDATORY DATA REQUIREMENTS BY EVENT TYPE:
%
%      A. FOR ALL EVENT TYPES (UNIVERSAL REQUIREMENTS):
%         □ Event name/title
%         □ Event date (DD/MM/YYYY format)
%         □ Event venue (specific location/room if applicable)
%         □ Event head/coordinator name
%         □ Report author name
%         □ Submission date
%         □ Total number of participants/registrations
%         □ Total number of attendees (actual attendance)
%         □ Event objectives (specific, measurable goals)
%         □ Event outcomes (actual results achieved)
%         □ At least 3-5 event photographs with descriptive contexts
%         □ Event schedule/agenda with specific timings
%
%      B. FOR WORKSHOPS/SEMINARS/GUEST LECTURES:
%         □ Resource person full name
%         □ Resource person designation/job title
%         □ Resource person organization/company name
%         □ Resource person email ID (mandatory for official records)
%         □ Resource person contact number (if available)
%         □ Resource person area of expertise
%         □ Resource person professional background/credentials
%         □ Topics/sessions handled by resource person
%         □ Duration of each session conducted
%
%      C. FOR COMPETITIONS/HACKATHONS/CONTESTS:
%         □ First place winner name/team name
%         □ First place winner department and year
%         □ First place project/solution title
%         □ Second place winner details (same as above)
%         □ Third place winner details (same as above)
%         □ Brief description of each winning solution
%         □ Prizes awarded to winners
%         □ Judging criteria used
%         □ Number of teams/individuals participated
%         □ Competition format and rules
%
%      D. FOR HACKATHONS (ADDITIONAL REQUIREMENTS):
%         □ Hackathon theme/problem statement
%         □ Technologies/tools allowed or recommended
%         □ Mentors/judges details (if external experts involved)
%         □ Project demonstration format
%         □ Evaluation metrics
%
%      E. FOR COLLABORATIVE EVENTS:
%         □ Collaboration partner organization name
%         □ Nature of collaboration
%         □ Resources/support provided by partner
%
%      HOW TO REQUEST MISSING DATA:
%      - Be SPECIFIC: Don't ask "provide more details" - ask for exact fields
%      - ORGANIZE requests by category: participant data, resource person info, etc.
%      - EXPLAIN why each field is needed: "Email ID is mandatory for official records"
%      - PRIORITIZE: Mark absolutely critical data vs. optional information
%      - PROVIDE format: "Please provide date in DD/MM/YYYY format"
%
%      EXAMPLE REQUEST FORMAT:
%      "I need the following information to complete your event report:
%
%      CRITICAL MISSING DATA:
%      1. Resource Person Details:
%         - Email ID (required for official records)
...
%         - Department-wise breakdown (CSE, IT, etc.)
%
%      3. Event Outcomes:
%         - Participant satisfaction rating (if feedback collected)
%
%      OPTIONAL BUT RECOMMENDED:
%      - Resource person contact number
%      - Quotes from participant feedback
%      - Follow-up activities planned
%
%      Please provide this information so I can generate a complete, final report
%      that requires no further updates."
%
%      DO NOT PROCEED with placeholder text like:
...
%      ✓ Wait for user to provide complete data
%      ✓ Then generate final, complete report
%
%      CONTENT GENERATION RULES - NO PLACEHOLDERS ANYWHERE:
%      - For paragraph content (Abstract, Introduction, sections), if specific
%        details are missing, use your domain knowledge to write professional,
%        generic, and contextually appropriate content
%      - Write in complete sentences with proper flow - never use bracketed
%        placeholders like [Add details here] or [To be filled]
%      - If event-specific data is unavailable, write general but professional
%        content relevant to that event type (e.g., for a workshop abstract,
%        discuss typical workshop benefits and learning outcomes)
%      - For tables and structured data fields, if critical data is missing,
%        STOP and REQUEST it - do not improvise table data
%      - Images can use example-image placeholders (example-image-a, etc.) as
%        the user will replace these with actual photos later
%
%      QUALITY CHECKS BEFORE GENERATING:
%      □ All tables have real data (no empty cells with placeholders)
%      □ All \newcommand definitions have actual values (no [brackets])
%      □ Abstract has complete sentences with flow (no placeholders)
%      □ All paragraph sections have proper prose (no [Add content here])
%      □ Event schedule table has real times and session names
%      □ Resource person table completely filled (if applicable)
%      □ Winners table completely filled (if applicable)
%      □ No bracketed placeholders anywhere in the document
%      □ Dates are in proper format throughout
%      □ Images can be example-image-a/b/c (will be replaced by user)
%
%      THE GOLDEN RULE:
%      "If critical structured data is missing, ASK. For paragraph content,
%      write professionally without placeholders. Never use [brackets] for
%      content that should be prose."
%      The report must be print-ready and submission-ready without any edits
%      to paragraph text. Only images need to be swapped by the user.
%
...
%
%   10. IMAGE HANDLING INSTRUCTIONS:
...
%       - For initial report generation, AI can use example-image-a, example-image-b,
%         example-image-c as placeholders - users will replace these with actual
%         event photos after uploading to Overleaf
%       - The only acceptable placeholders in the entire document are image
%         filenames (example-image-*) - all other content must be complete
%
...
%   - NEVER generate a report with bracketed placeholders [like this] in any
%     paragraph text, descriptions, or prose sections
%   - For missing structured data (tables, metadata), REQUEST information first
%   - For missing descriptive content, write professional generic content using
%     domain knowledge - never leave [placeholder brackets]
%   - The report must be FINAL and COMPLETE - only images use example-image-*
%     placeholders which users will replace
%
